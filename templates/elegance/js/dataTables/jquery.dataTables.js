...
(/** @lends <global> */function( $ ) {
	...
	function _fnGetCellData( settings, rowIdx, colIdx, type )
	{
		...
		var colnotif       = settings.oFeatures.notifcol;
		...
		if ( (cellData === rowData || cellData === null) && defaultContent !== null ) {
			...
			var $ladata = rowData[colnotif];
			
			if(colnotif !== null && $ladata.indexOf('isnotif') >= 0){
				cellData = defaultContent.replace('btn-inverse', 'btn-notif');
			}else{
				cellData = defaultContent;
			} 
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		var serch_e_1 = oSettings.oFeatures.search_extra1;
		        
		if(serch_e_1 != null) {
			ajax.data = function(d){
	
			    
		        $.each(serch_e_1, function( index, value ) {

		        	d[value.id] = $('#'+value.id).val();
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var jqFilter = $('#orig_search', filter)
	...
	{
		msg = 'DataTables Problème: '+
			(settings ? 'table id='+settings.sTableId+' - ' : '')+msg;
	
		if ( tn ) {
			msg += '. Contactez Administarteur'+tn;
		}
	...
	DataTable.defaults = {
		...
		"aoColumnDefs": [
                    {
                    	...
                    	aTargets: [-1],
                    	data: null,
                        defaultContent: '<div class="btn-group  btn_action"><button data-toggle="dropdown" class="btn btn-inverse btn-xs dropdown-toggle notif"><span class="ace-icon fa fa-caret-down icon-only"></span></button></div>',
                    },
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sProcessing": "<img src='img/loadin.gif'>Processing...",
			 ...
			 *    } );
			 */
			"sZeroRecords": "Aucune line trouvée"
		},
	...
	$.extend( DataTable.ext.classes, {
		...
		/* Filtering */
		"sFilterInput": "col-xs-10 col-sm-5",
...
function csv_export($table, $format)
{
    ...
    $.ajax({
                url: sUrl+'&lst=1&export=1&format=' + $format,
                type: 'POST',
                data: data,
                dataType: 'html',
                success: function(data) {
                	...
                	if(data_arry[0] == 1) {
        				if(data_arry[1] == 'CSV'){
        					...
        					$.fileDownload(data_arry[2]);
        				    bootbox.hideAll();
        				}else if(data_arry[1] == 'PDF'){
        					$.colorbox({iframe:true, width:"90%", height:"95%",href:data_arry[2]});
        				    bootbox.hideAll();
        				}
