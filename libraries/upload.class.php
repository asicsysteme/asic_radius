...
class Mupload {
     ...
     * Flag set after calling a process
     *
     * Indicates if the processing hase error reurn existour format
     *
     * @access public
     * @var bool
     */
    ...
    function init() {
        ...
        $this->stop_all                 = true;     // replace the name body
    ...
    function getsize($size) {
        ...
        $last_upper = $size{strlen($size)-1};
        $val_size = substr($size, 0, -1); 
        switch($last) {
            case 'g':
                $val_size *= 1024;
            case 'm':
        ...
        return $val_size.$size{strlen($size)-1};
    }
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if($this->stop_all == true){
                //$value_c = MInit::crypt_tp('f', $this->file_dst_pathname);
                    $result = array('path' => $this->file_dst_pathname);
                    echo json_encode($result);
                    exit();
                    //exit('1#'.$this->file_dst_pathname);
                }else{
            ...
            if($this->stop_all == true){
                    //exit('0#'. $this->error);
                    $result = array('message' => $this->error, 'error' => true);
                    echo json_encode($result);
                    exit();
                }else{
    ...
    function delete_file($file, $temp_file = true) {

        if($temp_file)
        {            
            if(file_exists($file) && !unlink($file))
            {
                $result = array('error' => false, 'message' => "Suppression temp_file impossible");
                echo json_encode($result);
                exit();
            }
            $result = array('error' => true, 'message' => "Suppression réussie");
            echo json_encode($result);
            exit();
        ...
        {
            $result = array('error' => false, 'message' => "Suppression Impossible (DB)");
            echo json_encode($result);
            exit(); // Stop All
        }else{
            if(!$db->RowCount()) 
        ...
        if(file_exists($file_link) && unlink($file_link)){
            ...
            if($this->stop_all == true){
                ...
                $db->Query("UPDATE $table SET $column = NULL WHERE id = $idm");//Updt PJ into Modul Table
                ...
                $this->log .= '- delete uploaded file '  . $file . '<br />';
            ...
            if($this->stop_all == true){
                ...
                $this->log .= '<b>Delete</b><br />';
                $this->log .= '- unable delete uploaded file '  . $file . '<br />';
