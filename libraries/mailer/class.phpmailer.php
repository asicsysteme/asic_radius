...
class PHPMailer
{
    ...
    /**
     * Function Hide email called for message show
     */

    public function hide_mail($email = '')
    {
        $mail_segments = explode("@", $email);
        $Tlld = explode(".",$mail_segments[1]);
        $first_char =  substr($mail_segments[0], 0, 2);
        $last_char =  substr($mail_segments[1], 0, 1);
        $Tlc = explode('.',$mail_segments[1]);

        $mail_segments[0] = $first_char.str_repeat("*", strlen($mail_segments[0])-strlen($first_char));
        $mail_segments[1] = $last_char.str_repeat("*", strlen($Tlc[0])-strlen($last_char)).'.'.$Tlc[1];
