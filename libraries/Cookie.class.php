...
// ---------------------------------------------------
// Last execution Application mais sans tik
// --------------------------------------------------- 
static public function auto_lastexec($field){
...
// Auto Logout
// ---------------------------------------------------
static public function auto_logout($field,$sec){
	if(isset($_SESSION[$field])){
    $t = time();
    $t0 = $_SESSION[$field];
    $diff = $t - $t0;
      if ($diff > $sec || !isset($t0)){          
        model::load('login','login');
	    //Stop session
	    stop_session();
		token::delimda();
		//$msg="Vous êtes déconnecté automatiquement pour inactivité de 5 min , redirection vers page login....  ";
		//exit(msgbox($msg,0,'./',5));     
        }
...
// ---------------------------------------------------
// On Start check Session open
// ---------------------------------------------------
static public function close_session_open()
{
global $db;
$req = "UPDATE session AS u
INNER JOIN
(
   SELECT lastactive AS lastactive, nom
   FROM users_sys where TIMESTAMPDIFF(MINUTE,users_sys.lastactive,NOW()) > 1
   GROUP BY nom
) AS l ON u.user = l.nom
SET u.expir = l.lastactive
where u.expir is NULL";	  
$db->Query( $req );
}

// ---------------------------------------------------
// Session Auto-closed return
// ---------------------------------------------------
static public function session_autoclosed()
{
global $db;
